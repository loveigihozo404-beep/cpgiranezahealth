...
function Courses() {
  ...
  return <>
    <PageIntro variant="courses-intro" eyebrow="Learning pathways" title="Programs that move with your ambition." text="Choose a focused pathway, build practical confidence and keep growing with CarePath Training Institute." />
...
function FAQ() { const faqContent = useSiteContent('content.faq', FAQ_DEFAULTS); const faqs = faqContent.items; const [open, setOpen] = useState(0); return <><PageIntro eyebrow="Questions, answered" title="A clearer start." text="Find quick answers about learning, support and working with CarePath Training Institute." /><section className="section"><div className="container faq-list">{faqs.map((item, i) => <div className={`faq-item ${open === i ? 'open' : ''}`} key={item.question}><button onClick={() => setOpen(open === i ? -1 : i)}><span>{item.question}</span><ChevronDown size={18} /></button>{open === i && <p>{item.answer}</p>}</div>)}</div></section></> }
function VerifyCertificate() {
  ...
  return <><PageIntro eyebrow="Certificate verification" title="Confirm a certificate with confidence." text="Enter a certificate number or verification code to check an issued CarePath Training Institute certificate." /><section className="section"><div className="container verify-box"><ShieldCheck size={38} /><h2>Certificate lookup</h2><p>Only enter the public certificate identifier. Private learner information is not displayed.</p><div className="verify-form"><input value={code} onChange={e => setCode(e.target.value)} placeholder="Certificate number or code" onKeyDown={e => e.key === 'Enter' && verify()} /><button className="button button-primary" onClick={verify} disabled={checking}>{checking ? 'Checking…' : 'Verify'}</button></div>{result && <div className={`verify-result ${result.found ? 'valid' : 'invalid'}`}>{result.found ? <><CheckCircle2 /><strong>Certificate Verified</strong><span>Holder: {result.name} · Course: {result.course} · Issued: {result.date}</span></> : <><X /><strong>Certificate Not Found</strong><span>No valid certificate matches that code. Contact CarePath Training Institute if you believe this is an error.</span></>}</div>}</div></section></>
}
    ...
    {submitted ? (
      <div className="auth-shell auth-shell--confirmation">
        <div className="auth-card confirmation-card">
          <Link to="/" className="brand auth-brand">{brand.logoUrl ? <img className="brand-logo-img" src={brand.logoUrl} alt="CarePath Training Institute" /> : <span className="brand-mark"><HeartPulse size={21} /></span>}<span>CarePath Training Institute</span></Link>
        ...
        {isRegister && (
          <aside className="auth-visual-panel">
            ...
            <div className="auth-visual-content">
              ...
              <p>Join CarePath Training Institute today and access quality healthcare services with ease.</p>
...
function ResetPassword() {
  ...
  return <section className="auth-page"><div className="auth-shell"><div className="login-title">CarePath Training Institute</div><div className="auth-card login-card">
        ...
        <aside className={`student-sidebar ${sidebarOpen ? 'open' : ''}`}>
          <div className="student-sidebar-header">
            <Link className="brand student-brand" to="/" onClick={() => setSidebarOpen(false)}>
              ...
              <span>CarePath Training Institute</span>
            </Link>
